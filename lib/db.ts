...
export async function initDB(): Promise<void> {
  const db = getDB();
  await db.batch([
    `CREATE TABLE IF NOT EXISTS indicators (id TEXT PRIMARY KEY, type TEXT NOT NULL CHECK(type IN ('domain','ipv4-addr','ipv6-addr','url','email-addr','file-hash')), value TEXT NOT NULL, value_normalized TEXT NOT NULL, risk_level TEXT NOT NULL DEFAULT 'unknown' CHECK(risk_level IN ('known_malicious','high_risk','medium_risk','low_risk','clean','unknown')), confidence INTEGER NOT NULL DEFAULT 50 CHECK(confidence BETWEEN 0 AND 100), tlp TEXT NOT NULL DEFAULT 'white' CHECK(tlp IN ('white','green','amber','red')), categories TEXT DEFAULT '[]', tags TEXT DEFAULT '[]', description TEXT, source_feed TEXT NOT NULL, source_ref TEXT, first_seen TEXT NOT NULL DEFAULT (datetime('now')), last_seen TEXT NOT NULL DEFAULT (datetime('now')), valid_from TEXT NOT NULL DEFAULT (datetime('now')), valid_until TEXT, kill_chain_phase TEXT, threat_actor TEXT, campaign TEXT, hit_count INTEGER NOT NULL DEFAULT 0, last_hit_at TEXT, is_active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')))`,
    ...
    `CREATE TABLE IF NOT EXISTS feeds (id TEXT PRIMARY KEY, name TEXT NOT NULL, url TEXT NOT NULL, feed_type TEXT NOT NULL DEFAULT 'osint' CHECK(feed_type IN ('osint','commercial','internal','community')), indicator_type TEXT NOT NULL DEFAULT 'domain', parser TEXT NOT NULL DEFAULT 'text_lines', is_active INTEGER NOT NULL DEFAULT 1, enabled INTEGER NOT NULL DEFAULT 1, refresh_interval_min INTEGER NOT NULL DEFAULT 15, last_refresh TEXT, last_success TEXT, last_error TEXT, entries_count INTEGER NOT NULL DEFAULT 0, total_ingested INTEGER NOT NULL DEFAULT 0, avg_refresh_ms INTEGER DEFAULT 0, status TEXT NOT NULL DEFAULT 'pending' CHECK(status IN ('active','error','pending','disabled')), config TEXT DEFAULT '{}', created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')))`,
    `CREATE TABLE IF NOT EXISTS ingestion_log (id INTEGER PRIMARY KEY AUTOINCREMENT, feed_id TEXT NOT NULL, started_at TEXT NOT NULL DEFAULT (datetime('now')), completed_at TEXT, status TEXT NOT NULL DEFAULT 'running' CHECK(status IN ('running','success','error','partial')), indicators_added INTEGER DEFAULT 0, indicators_updated INTEGER DEFAULT 0, indicators_removed INTEGER DEFAULT 0, duration_ms INTEGER, error_message TEXT, FOREIGN KEY (feed_id) REFERENCES feeds(id))`,
    ...
    `CREATE TABLE IF NOT EXISTS custom_url_categories (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE, description TEXT, action TEXT NOT NULL DEFAULT 'Block' CHECK(action IN ('Block','Warn','Allow','Isolate')), is_active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')))`,
    `CREATE TABLE IF NOT EXISTS custom_url_entries (id INTEGER PRIMARY KEY AUTOINCREMENT, category_id INTEGER NOT NULL, domain TEXT NOT NULL, added_by TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')), FOREIGN KEY (category_id) REFERENCES custom_url_categories(id))`,
    `CREATE UNIQUE INDEX IF NOT EXISTS idx_cue_domain_cat ON custom_url_entries(domain, category_id)`,
    `CREATE TABLE IF NOT EXISTS url_policy_overrides (id INTEGER PRIMARY KEY AUTOINCREMENT, domain TEXT NOT NULL UNIQUE, action TEXT NOT NULL CHECK(action IN ('Block','Warn','Allow','Isolate')), reason TEXT, added_by TEXT, expires_at TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')))`,
    ...
    `CREATE TABLE IF NOT EXISTS url_policy_group_rules (id INTEGER PRIMARY KEY AUTOINCREMENT, group_id INTEGER NOT NULL, category TEXT NOT NULL, action TEXT NOT NULL CHECK(action IN ('Block','Warn','Allow','Isolate')), FOREIGN KEY (group_id) REFERENCES url_policy_groups(id))`,
    `CREATE TABLE IF NOT EXISTS url_policy_user_assignments (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id TEXT NOT NULL, group_id INTEGER NOT NULL, assigned_at TEXT NOT NULL DEFAULT (datetime('now')), FOREIGN KEY (group_id) REFERENCES url_policy_groups(id))`,
    `CREATE UNIQUE INDEX IF NOT EXISTS idx_upa_user_group ON url_policy_user_assignments(user_id, group_id)`,
    `CREATE TABLE IF NOT EXISTS url_policy_schedules (id INTEGER PRIMARY KEY AUTOINCREMENT, group_id INTEGER NOT NULL, days_of_week TEXT NOT NULL DEFAULT '["mon","tue","wed","thu","fri"]', start_time INTEGER NOT NULL DEFAULT 900, end_time INTEGER NOT NULL DEFAULT 1800, is_active INTEGER NOT NULL DEFAULT 1, FOREIGN KEY (group_id) REFERENCES url_policy_groups(id))`,
