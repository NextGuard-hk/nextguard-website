...
export function NewsFeedPage() {
  ...
  const hdr:Record<string,{headline:string;subheadline:string}>={en:{headline:"AI Cybersecurity News Feed",subheadline:"Real-time AI-curated cybersecurity intelligence from trusted sources"},"zh-TW":{headline:"AI \u7db2\u7d61\u5b89\u5168\u65b0\u805e",subheadline:"\u5373\u6642 AI \u7be9\u9078\u5168\u7403\u53ef\u4fe1\u4f86\u6e90\u7684\u7db2\u7d61\u5b89\u5168\u60c5\u5831"},"zh-CN":{headline:"AI \u7f51\u7edc\u5b89\u5168\u65b0\u95fb",subheadline:"\u5b9e\u65f6 AI \u7b5b\u9009\u5168\u7403\u53ef\u4fe1\u6765\u6e90\u7684\u7f51\u7edc\u5b89\u5168\u60c5\u62a5"}}
  const h=hdr[language]||hdr.en
  return (<>
    <PageHeader badge="AI News" headline={h.headline} subheadline={h.subheadline} />
    <section className="py-8 px-4 max-w-6xl mx-auto">
      ...
      {loading?(<div className="flex justify-center py-20"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div>):articles.length===0?(<div className="text-center py-20 text-muted-foreground">{language==="en"?"No articles found. Check back soon!":"\u627e\u4e0d\u5230\u6587\u7ae0"}</div>):(<>
        <div className="space-y-4">
          {articles.map((a,i)=>(<AnimateIn key={a.id} delay={i*50}><a href={a.url} target="_blank" rel="noopener noreferrer" className="block p-5 rounded-xl bg-card border border-border/50 hover:border-primary/30 hover:bg-card/80 transition-all group"><div className="flex items-center gap-2 mb-2"><Rss className="w-4 h-4 text-muted-foreground" /><span className="text-sm text-muted-foreground">{a.source}</span><span className="ml-auto"><span className={`text-[10px] px-2 py-0.5 rounded-full ${importBadge[a.importance]}`}>{a.importance}</span></span></div><h3 className="text-lg font-semibold text-foreground group-hover:text-primary transition-colors mb-2">{a.title}</h3><p className="text-sm text-muted-foreground line-clamp-2 mb-3">{a.summary}</p><div className="flex items-center justify-between"><div className="flex gap-1.5 flex-wrap">{a.tags.slice(0,3).map(t=>(<span key={t} className={`text-[10px] px-2 py-0.5 rounded-full border ${tagColors[t]||"bg-muted text-muted-foreground border-muted"}`}>{t}</span>))}</div><span className="text-xs text-muted-foreground">{new Date(a.publishedAt).toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"})} &middot; {a.source}</span></div></a></AnimateIn>))}
        </div>
        {tp>1&&(<div className="flex items-center justify-center gap-4 mt-8"><button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1} className="px-4 py-2 rounded-lg bg-muted text-sm font-medium disabled:opacity-50 hover:bg-muted/80">Previous</button><span className="text-sm text-muted-foreground">{page}/{tp}</span><button onClick={()=>setPage(p=>Math.min(tp,p+1))} disabled={page===tp} className="px-4 py-2 rounded-lg bg-muted text-sm font-medium disabled:opacity-50 hover:bg-muted/80">Next</button></div>)}
      </>)}
    </section>
  </>)
}
