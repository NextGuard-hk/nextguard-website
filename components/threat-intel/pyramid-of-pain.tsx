...
// David Bianco's Pyramid of Pain visualization — uses shared context
'use client';
...
export default function PyramidOfPain() {
  const { stats, loading } = useThreatIntelData();

  const counts = useMemo(() => {
    const byType: Record<string, number> = {};
    (stats?.by_type || []).forEach((t: any) => {
      const type = (t.type || '').toLowerCase();
      if (type.includes('hash') || type === 'md5' || type === 'sha1' || type === 'sha256') byType.hash = (byType.hash || 0) + t.count;
      else if (type.includes('ip') || type === 'ipv4' || type === 'ipv6' || type === 'ipv4-addr') byType.ipv4 = (byType.ipv4 || 0) + t.count;
      ...
      else if (type.includes('email')) byType.network = (byType.network || 0) + t.count;
      else byType.tools = (byType.tools || 0) + t.count;
    });
    return byType;
  }, [stats]);
