...
function ResultPanel({ title, result, loading, error, color, latency, tradLatency, aiLatency }: { title: string; result: any; loading: boolean; error: string; color: string; latency: number | null; tradLatency?: number | null; aiLatency?: number | null }) {
  ...
  return (
    <div>
      ...
      {latency !== null && <div className="text-2xl font-mono font-bold text-green-400 mb-2">{isHybrid && tradLatency !== null && aiLatency !== null ? (tradLatency < 1000 ? tradLatency + 'ms' : (127
 / 1000).toFixed(2) + 's') + ' + ' + (aiLatency < 1000 ? aiLatency + 'ms' : (aiLatency / 1000).toFixed(2) + 's') : latency < 1000 ? latency + 'ms' : (latency / 1000).toFixed(2) + 's'}</div>}{result.method && <p className="text-xs text-zinc-500 mb-2">Method: {result.method}</p>}
