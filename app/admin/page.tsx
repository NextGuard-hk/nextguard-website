          ...
          <div>
            ...
            {accountsLoading && accounts.length === 0 ? <p className="text-zinc-400">Loading accounts...</p> : accounts.length === 0 ? <p className="text-zinc-400">No registered accounts yet.</p> : (
              <div className="overflow-x-auto">
                ...
                <td className="py-3 pr-4">{[{key:'kb',label:'Knowledge Base'},{key:'download',label:'Downloads'},{key:'socReview',label:'AI SOC'},{key:'projectAccess',label:'Project Access'},{key:'quotation',label:'Quotation'},{key:'pocLicense',label:'PoC License'},{key:'productionLicense',label:'Prod License'}].map(p => <button key={p.key} onClick={() => { const perms = a.permissions || {kb:false,download:true,socReview:false}; const updated = {...perms, [p.key]: !perms[p.key as keyof typeof perms]}; setAccounts((prev: any) => prev.map((acc: any) => acc.id === a.id ? {...acc, permissions: updated} : acc)); fetch('/api/download-users?secret=nextguard-cron-2024-secure&action=set-permissions&id=' + encodeURIComponent(a.id) + '&permissions=' + encodeURIComponent(JSON.stringify(updated)), {method:'PUT'}).then(r => { if(r.ok) fetchAccounts(); else alert('Failed to update permission') }) }} className={((a.permissions || {kb:false,download:true,socReview:false})[p.key as keyof typeof a.permissions] ? 'bg-green-600 text-white' : 'bg-red-900/50 text-red-400') + ' px-3 py-1 rounded-md text-xs font-medium mr-1.5 cursor-pointer transition-colors'}>{p.label}</button>)}</td>
                      <td className="py-3">
