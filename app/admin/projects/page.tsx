...
export default function ProjectsPage() {
  ...
  const NL = String.fromCharCode(10); const BOM = String.fromCharCode(0xFEFF); const handleExportActivityLog = () => { const h = ['Time','User','Key','Title','Action','Field','Old Value','New Value']; const NL = String.fromCharCode(10); const BOM = String.fromCharCode(0xFEFF); const esc = (v: string) => { const s = String(v||''); return (s.includes(',') || s.includes('"')) ? '"' + s.replace(/"/g, '""') + '"' : s; }; const rows = activityLogs.map(l => [l.timestamp,l.user,l.issueKey,l.issueTitle,l.action,l.field||'',l.oldValue||'',l.newValue||''].map(esc).join(',')); const csv = BOM + h.join(',') + NL + rows.join(NL); const blob = new Blob([csv], { type: 'application/vnd.ms-excel;charset=utf-8' }); const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'ActivityLog_' + new Date().toISOString().split('T')[0] + '.xls'; a.click(); URL.revokeObjectURL(u); }; const handleExportExcel = () => { const h = ['Key','Title','Type','Status','Priority','Assignee','Department','Sprint','Points','Labels','Due Date','Due Time','Created','Updated']; const esc = (v: string) => { const s = String(v); return (s.includes(',') || s.includes('"')) ? '"' + s.replace(/"/g, '""') + '"' : s; }; const rows = filteredIssues.map(i => [i.key,i.title,i.type,i.status,i.priority,getUserName(i.assignee),i.department,i.sprint,String(i.points),i.labels.join('; '),i.dueDate||'',i.dueTime||'',i.created,i.updated].map(esc).join(',')); const csv = BOM + h.join(',') + NL + rows.join(NL); const blob = new Blob([csv], { type: 'application/vnd.ms-excel;charset=utf-8' }); const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'NextGuard_Tasks_' + new Date().toISOString().split('T')[0] + '.xls'; a.click(); URL.revokeObjectURL(u); }; // ==================== KB 3-LAYER CRUD ====================
  ...
  <div className="min-h-screen bg-gray-950 text-white flex">
    ...
    {showActivityLog && (
      <div className="fixed inset-0 z-[80] flex items-center justify-center p-4" onClick={() => setShowActivityLog(false)}><div className="absolute inset-0 bg-black/60" />
        <div className="relative bg-gray-900 rounded-xl border border-gray-800 w-full max-w-3xl max-h-[85vh] overflow-hidden flex flex-col"><div className="flex items-center justify-between px-4 py-3 border-b border-gray-800"><h3 className="text-lg font-semibold text-white">📝 Activity Log</h3><div className="flex items-center gap-2"><button onClick={handleExportActivityLog} className="px-3 py-1.5 bg-green-600 hover:bg-green-500 rounded-lg text-xs font-medium transition-colors">📊 Export Excel</button><button onClick={() => setShowActivityLog(false)} className="text-gray-400 hover:text-white text-xl leading-none">✕</button></div></div>" onClick={e => e.stopPropagation()}>
