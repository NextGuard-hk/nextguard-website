        ...
        <div className="fixed inset-0 z-[90] flex items-center justify-center p-4" onClick={() => setShowEditModal(false)}>
          ...
          <div className="relative bg-gray-900 rounded-xl border border-gray-800 w-full max-w-lg p-4 sm:p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div className="block text-sm text-gray-400 mb-1">Department</label><select value={editIssue.department} onChange={e => setEditIssue({...editIssue,department:e.target.value as Department})} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm">{departments.map(d => <option key={d}>{d}</option>)}</select></div></div><div className="grid grid-cols-2 gap-3"><div><label className="block text-sm text-gray-400 mb-1">Due Date</label><input type="date" value={editIssue.dueDate || ''} onChange={e => setEditIssue({...editIssue,dueDate:e.target.value})} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:border-cyan-500 focus:outline-none" /></div><div><label className="block text-sm text-gray-400 mb-1">Due Time</label><input type="time" value={editIssue.dueTime || ''} onChange={e => setEditIssue({...editIssue,dueTime:e.target.value})} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:border-cyan-500 focus:outline-none" /></div></div><div className="flex gap-2 pt-2"><button onClick={handleEditIssue} className="flex-1 py-2 bg-cyan-600 hover:bg-cyan-500 rounded-lg font-medium text-sm transition-colors">Save Changes</button><button onClick={() => setShowEditModal(false)} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-sm transition-colors">Cancel</button></div>
            </div>
          </div>
        </div>
      )}
