        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4" onClick={() => setSelectedIssue(null)}>
          ...
          <div className="relative bg-gray-900 rounded-xl border border-gray-800 w-full max-w-2xl max-h-[90vh] overflow-y-auto p-4 sm:p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-2 mb-4 flex-wrap">
              ...
              {selectedIssue.status !== 'Done' && selectedIssue.status !== 'Closed' && <button onClick={() => { const done = {...selectedIssue, status: 'Done' as Status, updated: new Date().toISOString().split('T')[0]}; setIssues(prev => prev.map(i => i.id === selectedIssue.id ? done : i)); setSelectedIssue(done); }} className="px-3 py-1.5 bg-green-600 hover:bg-green-500 rounded-lg text-xs font-medium transition-colors">✅ Done</button>} {selectedIssue.status !== 'Closed' ? (<button onClick={handleCloseIssue} className="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 rounded-lg text-xs font-medium transition-colors">Close</button>) : (<button onClick={handleReopenIssue} className="px-3 py-1.5 bg-yellow-600 hover:bg-yellow-500 rounded-lg text-xs font-medium transition-colors">Reopen</button>)}
            ...
            <div className="grid grid-cols-2 gap-3 text-sm mb-4">
              ...
              <div><span className="text-gray-500">Points: </span><span>{selectedIssue.points}</span></div><div><span className="text-gray-500">Attachments: </span><span>{selectedIssue.attachments} file(s)</span>{selectedIssue.attachmentFiles && selectedIssue.attachmentFiles.length > 0 && <div className="col-span-2 mt-1 space-y-1">{selectedIssue.attachmentFiles.map((f,i) => <a key={i} href={f.url} download={f.name} className="flex items-center gap-2 text-xs bg-gray-800 p-2 rounded hover:bg-gray-700 cursor-pointer"><span className="text-cyan-400">📎</span><span className="text-cyan-300 hover:underline">{f.name}</span><span className="text-gray-500 ml-auto">{(f.size/1024).toFixed(1)}KB</span></a>)}</div>}</div>
