                ...
                <div className="space-y-2">
                  {ci.map(issue => (
                    <div key={issue.id} onClick={() => setSelectedIssue(issue)} className="bg-gray-800 border border-gray-700 rounded-lg p-3 cursor-pointer hover:border-cyan-500/50 transition-colors">
                      <div className="flex items-center gap-1.5 mb-1.5 flex-wrap"><span className="text-xs">{(typeConfig[issue.type] || typeConfig.Task).icon}</span><span className="text-xs text-gray-500 font-mono">{issue.key}</span><span className={`text-xs ${(priorityConfig[issue.priority] || priorityConfig.Medium).bg} ${(priorityConfig[issue.priority] || priorityConfig.Medium).color} px-1.5 rounded`}>{issue.priority}</span></div>
        ...
        {activeView === 'list' && (
          <div className="bg-gray-900/50 rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>{filteredIssues.map(issue => (<tr key={issue.id} onClick={() => setSelectedIssue(issue)} className="border-b border-gray-800/50 hover:bg-gray-800/50 cursor-pointer"><td className="p-3 font-mono text-cyan-400 text-xs">{issue.key}</td><td className="p-3">{issue.title}</td><td className="p-3"><span className="text-xs">{(typeConfig[issue.type] || typeConfig.Task).icon} {issue.type}</span></td><td className="p-3"><span className="text-xs bg-gray-800 px-2 py-0.5 rounded">{issue.status}</span></td><td className="p-3"><span className={`text-xs ${(priorityConfig[issue.priority] || priorityConfig.Medium).color}`}>{(priorityConfig[issue.priority] || priorityConfig.Medium).icon} {issue.priority}</span></td><td className="p-3 text-sm">{getUserName(issue.assignee)}</td><td className="p-3 text-xs text-gray-500">{issue.dueDate || '-'}{issue.dueTime ? ` ${issue.dueTime}` : ''}</td><td className="p-3">{issue.points}</td></tr>))}</tbody>
            </table>
          </div>
        )}
        {activeView === 'backlog' && (
          <div className="space-y-6">
            {['Sprint 12','Sprint 13','Backlog'].map(sprint => { const si = filteredIssues.filter(i => sprint === 'Backlog' ? (i.status === 'Backlog' && i.sprint !== 'Sprint 12' && i.sprint !== 'Sprint 13') : i.sprint === sprint); return (
              <div key={sprint} className="bg-gray-900/50 rounded-xl p-4">
                ...
                <div className="space-y-1">{si.map(issue => (<div key={issue.id} onClick={() => setSelectedIssue(issue)} className="flex items-center gap-2 sm:gap-4 py-2 px-2 sm:px-3 hover:bg-gray-800 rounded-lg cursor-pointer"><span className="text-sm">{(typeConfig[issue.type] || typeConfig.Task).icon}</span><span className="text-xs font-mono text-cyan-400 w-16">{issue.key}</span><span className="text-sm flex-1 truncate">{issue.title}</span><span className={`text-xs ${(priorityConfig[issue.priority] || priorityConfig.Medium).color}`}>{issue.priority}</span><span className="text-xs text-gray-500">{issue.points} pts</span></div>))}</div>
              </div>
            ); })}
          </div>
        )}
