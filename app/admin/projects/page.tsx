  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex">
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-hidden">
        ...
        {activeView === 'kb' && (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">{mockKBArticles.map(a=>(<div key={a.id} onClick={()=>setSelectedKBArticle(a)} className="bg-gray-900 rounded-xl border border-gray-800 p-4 sm:p-6 hover:border-cyan-500/30 transition-colors cursor-pointer"><div className="flex items-center gap-2 mb-2"><span className="text-xs bg-cyan-500/10 text-cyan-400 px-2 py-0.5 rounded">{a.category}</span><span className="text-xs bg-gray-800 text-gray-400 px-2 py-0.5 rounded">{a.department}</span></div><h3 className="font-semibold text-white mb-2">{a.title}</h3><p className="text-sm text-gray-400 line-clamp-3 mb-3">{a.content}</p><div className="flex items-center justify-between text-xs text-gray-500"><span>By {getUserName(a.author)}</span><span>{a.views} views</span></div></div>))}</div>)}
        </div>
