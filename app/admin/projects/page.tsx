      ...
      <div className="fixed inset-0 z-[80] flex items-center justify-center p-4" onClick={() => setSelectedIssue(null)}><div className="absolute inset-0 bg-black/60" />
        <div className="relative bg-gray-900 rounded-xl border border-gray-800 w-full max-w-2xl max-h-[90vh] overflow-y-auto p-4 sm:p-6" onClick={e => e.stopPropagation()}>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm mb-4">
            ...
            <div><span className="text-gray-500">Points: </span><span>{selectedIssue.points}</span></div><div><span className="text-gray-500">Due: </span><span>{selectedIssue.dueDate || 'Not set'}{selectedIssue.dueTime ? ` ${selectedIssue.dueTime}` : ''}</span></div>
          </div>
          ...
          <div className="text-xs text-gray-600"><span>{selectedIssue.attachmentFiles && selectedIssue.attachmentFiles.length > 0 && <div className="mt-4 border-t border-gray-800 pt-3"><label className="block text-sm text-gray-400 mb-2">Attachments ({selectedIssue.attachmentFiles.length})</label><div className="space-y-1">{selectedIssue.attachmentFiles.map((f,i) => <div key={i} className="flex items-center justify-between px-3 py-2 bg-gray-800 rounded-lg text-sm"><div className="flex items-center gap-2 truncate"><span>{f.name.match(/\.(png|jpg|jpeg|gif|svg|webp)$/i) ? '🖼' : f.name.match(/\.pdf$/i) ? '📄' : '📎'}</span><span className="truncate">{f.name}</span><span className="text-xs text-gray-500">({(f.size/1024).toFixed(1)}KB)</span></div><div className="flex gap-2"><button onClick={() => window.open(f.url,'_blank')} className="text-cyan-400 hover:text-cyan-300 text-xs">View</button><a href={f.url} download={f.name} className="text-green-400 hover:text-green-300 text-xs">Download</a></div></div>)}</div></div>}<span>Created: {selectedIssue.created}</span><span className="mx-2">|</span><span>Updated: {selectedIssue.updated}</span></div>
        </div>
      </div>
    )}
    {/* Edit Issue Modal */}
