    ...
    {showActivityLog && (
      <div className="fixed inset-0 z-[80] flex items-center justify-center p-4" onClick={() => setShowActivityLog(false)}><div className="absolute inset-0 bg-black/60" />
        <div className="relative bg-gray-900 rounded-xl border border-gray-800 w-full max-w-3xl max-h-[85vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
          <div className="flex items-center justify-between p-4 border-b border-gray-800"><h3 className="text-lg font-semibold">Activity Log ({activityLogs.length})</h3><button onClick={() => <button onClick={()=>{const B=String.fromCharCode(0xFEFF);const N=String.fromCharCode(10);const h=['Action','Issue Key','Title','Field','Old Value','New Value','User','Timestamp'];const r=activityLogs.map((l:any)=>[l.action,l.issueKey,l.issueTitle,l.field||'',l.oldValue||'',l.newValue||'',l.user,l.timestamp].map((v:any)=>{const s=String(v);return(s.includes(',')||s.includes('"'))?'"'+s.replace(/"/g,'""')+'"':s;}).join(','));const c=B+h.join(',')+N+r.join(N);const bl=new Blob([c],{type:'application/vnd.ms-excel;charset=utf-8'});const u=URL.createObjectURL(bl);const a=document.createElement('a');a.href=u;a.download='ActivityLog_'+new Date().toISOString().split('T')[0]+'.xls';a.click();URL.revokeObjectURL(u);}} className="px-3 py-1.5 bg-green-600 hover:bg-green-500 rounded-lg text-xs font-medium mr-2 transition-colors">📥 Export XLS</button><button onClick={()=>setShowActivityLog(false)} className="text-gray-400 hover:text-white text-xl">X</button></div>
