...
type ViewMode = 'board' | 'list' | 'backlog' | 'reports' | 'kb' | 'accounts';
...
export default function ProjectsPage() {
  ...
  const handleVerifyOTP = async () => { setLoginLoading(true); setLoginError(''); try { const r = await fetch('/api/projects/auth', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: emailInput, otp: otpInput }) }); const d = await r.json(); if (r.ok && d.success) { setCurrentUser({ id: d.user.id, name: d.user.name, email: d.user.email, department: 'R&D' as Department, role: 'User', avatar: d.user.name.split(' ').map((n: string) => n[0]).join('') }); setIsLoggedIn(true); setOtpRequired(false); setOtpInput(''); } else { setLoginError(d.error || 'Invalid OTP'); } } catch { setLoginError('Network error'); } finally { setLoginLoading(false); } }; const const handleRegister = async () => {
    ...
    try {
      const r = await fetch('/api/projects/auth', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ action: 'register', name: regName, email: regEmail, password: regPassword }) });
      const d = await r.json();
      if (r.ok && d.success) { setRegSuccess(d.message || 'Registration successful!'); setTimeout(() => { setLoginMode('login'); setRegSuccess(''); }, 2000); }
      else { setRegError(d.error || 'Registration failed'); }
  ...
  if (!isLoggedIn) {
    ...
    <div className="min-h-screen bg-gray-950 text-white flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="flex items-center justify-center gap-3 mb-4"><img src="/nextguard-logo.svg" alt="NextGuard" className="w-10 h-10" onError={(e) => { (e.target as HTMLImageElement).style.display='none' }} /><span className="text-xl font-bold text-white">NextGuard DLP</span></div><h2 className="text-2xl font-bold text-center mb-2">Project Management Platform</h2><p className="text-gray-400 text-center text-sm mb-6">{loginMode === 'register' ? 'Create a new account' : loginMode === 'forgot' ? 'Reset your password' : 'Sign in with your account'}</p>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          <h3 className="text-lg font-semibold mb-4">{loginMode === 'register' ? 'Create Account' : loginMode === 'forgot' ? 'Reset Password' : otpRequired ? 'Enter OTP' : 'Sign In'}</h3>
          {loginMode === 'login' && (<>{loginError && <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 rounded-lg text-red-400 text-sm">{loginError}</div>}
          <div className="space-y-3">
            ...
            <button onClick={otpRequired ? handleVerifyOTP : handleLogin} disabled={loginLoading} className="w-full py-3 bg-cyan-600 hover:bg-cyan-500 rounded-lg font-medium transition-colors disabled:opacity-50">{loginLoading ? 'Verifying...' : otpRequired ? 'Verify OTP' : 'Sign In'}</button>{otpRequired && <div className="mt-3"><label className="block text-sm text-gray-400 mb-1">Enter OTP Code</label><input type="text" value={otpInput} onChange={e => setOtpInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleVerifyOTP()} placeholder="Enter 6-digit OTP" className="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-cyan-500 focus:outline-none text-center text-2xl tracking-widest" maxLength={6} /></div>}
              </>)}
          </div>
          <p className="text-xs text-gray-500 mt-4 text-center">{loginMode === 'login' && <p className="text-xs text-gray-500 mt-4 text-center">Don't have an account? <button onClick={() => { setLoginMode('register'); setLoginError(''); }} className="text-cyan-400 hover:text-cyan-300 underline">Register</button> | <button onClick={() => { setLoginMode('forgot'); setLoginError(''); }} className="text-cyan-400 hover:text-cyan-300 underline">Forgot Password?</button></p>}{loginMode === 'register' && <div className="mt-3 space-y-3"><div><label className="block text-sm text-gray-400 mb-1">Full Name</label><input type="text" value={regName} onChange={e => setRegName(e.target.value)} placeholder="Your name" className="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-cyan-500 focus:outline-none" /></div><div><label className="block text-sm text-gray-400 mb-1">Email</label><input type="email" value={regEmail} onChange={e => setRegEmail(e.target.value)} placeholder="your@email.com" className="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-cyan-500 focus:outline-none" /></div><div><label className="block text-sm text-gray-400 mb-1">Password</label><input type="password" value={regPassword} onChange={e => setRegPassword(e.target.value)} placeholder="Min 6 characters" className="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-cyan-500 focus:outline-none" /></div><div><label className="block text-sm text-gray-400 mb-1">Confirm Password</label><input type="password" value={regConfirm} onChange={e => setRegConfirm(e.target.value)} placeholder="Confirm password" className="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-cyan-500 focus:outline-none" /></div>{regError && <p className="text-red-400 text-sm">{regError}</p>}{regSuccess && <p className="text-green-400 text-sm">{regSuccess}</p>}<button onClick={handleRegister} disabled={regLoading} className="w-full py-3 bg-cyan-600 hover:bg-cyan-500 rounded-lg font-medium transition-colors disabled:opacity-50">{regLoading ? 'Registering...' : 'Create Account'}</button><p className="text-xs text-gray-500 mt-2 text-center">Already have an account? <button onClick={() => setLoginMode('login')} className="text-cyan-400 hover:text-cyan-300 underline">Sign In</button></p></div>}{loginMode === 'forgot' && <div className="mt-3 space-y-3"><div><label className="block text-sm text-gray-400 mb-1">Email Address</label><input type="email" value={forgotEmail} onChange={e => setForgotEmail(e.target.value)} placeholder="your@email.com" className="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-cyan-500 focus:outline-none" /></div>{forgotError && <p className="text-red-400 text-sm">{forgotError}</p>}{forgotSuccess && <p className="text-green-400 text-sm">{forgotSuccess}</p>}<button onClick={handleForgotPassword} disabled={forgotLoading} className="w-full py-3 bg-cyan-600 hover:bg-cyan-500 rounded-lg font-medium transition-colors disabled:opacity-50">{forgotLoading ? 'Sending...' : 'Reset Password'}</button><p className="text-xs text-gray-500 mt-2 text-center">Remember your password? <button onClick={() => setLoginMode('login')} className="text-cyan-400 hover:text-cyan-300 underline">Sign In</button></p></div>}{loginMode === 'login' && <p className="text-xs text-gray-500 mt-4 text-center">Use your registered account to sign in. Contact admin if you need Project Access permission.</p>}
        </div>
      </div>
    </div>
    );
  ...
  <div className="min-h-screen bg-gray-950 text-white flex">
    ...
    <aside className={`${sidebarCollapsed ? 'w-16' : 'w-64'} ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 fixed lg:sticky top-0 h-screen bg-gray-900 border-r border-gray-800 flex flex-col z-50 transition-all`}>
      ...
      <nav className="flex-1 p-3 space-y-1">
        {([{key:'board' as ViewMode,label:'Board',icon:'📊'},{key:'list' as ViewMode,label:'List View',icon:'📃'},{key:'backlog' as ViewMode,label:'Backlog',icon:'📥'},{key:'reports' as ViewMode,label:'Reports',icon:'📈'},{key:'kb' as ViewMode,label:'Knowledge Base',icon:'📚'},{key:'accounts' as ViewMode,label:'Accounts',icon:'👥'}]).map(item => (
          <button key={item.key} onClick={() => { setActiveView(item.key); if (item.key === 'accounts') loadAccounts(); setMobileSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors ${activeView === item.key ? 'bg-cyan-600/20 text-cyan-400' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}>
    ...
    {activeView === 'accounts' && (<div className="p-4 sm:p-6 space-y-4"><div className="flex items-center justify-between"><h2 className="text-xl font-bold">Project Platform Accounts</h2><button onClick={() => { setShowCreateAccount(true); }} className="px-4 py-2 bg-cyan-600 hover:bg-cyan-500 rounded-lg text-sm font-medium">+ Create Account</button></div><div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden"><table className="w-full text-sm"><thead><tr className="border-b border-gray-800 text-gray-400"><th className="text-left p-3">Name</th><th className="text-left p-3">Email</th><th className="text-left p-3">Role</th><th className="text-left p-3">Status</th><th className="text-left p-3">Created</th><th className="text-left p-3">Actions</th></tr></thead><tbody>{accounts.map((acc: any) => (<tr key={acc.id} className="border-b border-gray-800/50 hover:bg-gray-800/30"><td className="p-3 font-medium">{acc.name}</td><td className="p-3 text-gray-400">{acc.email}</td><td className="p-3"><span className={`px-2 py-1 rounded text-xs ${acc.role === 'admin' ? 'bg-purple-500/20 text-purple-400' : 'bg-blue-500/20 text-blue-400'}`}>{acc.role}</span></td><td className="p-3"><span className={`px-2 py-1 rounded text-xs ${acc.active ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>{acc.active ? 'Active' : 'Inactive'}</span></td><td className="p-3 text-gray-500">{acc.created}</td><td className="p-3"><button onClick={() => handleDeleteAccount(acc.id)} className="text-red-400 hover:text-red-300 text-xs">Delete</button></td></tr>))}</tbody></table>{accounts.length === 0 && <p className="text-center text-gray-500 py-8">No accounts yet. Create one to get started.</p>}</div>{showCreateAccount && (<div className="fixed inset-0 z-[80] flex items-center justify-center p-4" onClick={() => setShowCreateAccount(false)}><div className="absolute inset-0 bg-black/60" /><div className="relative bg-gray-900 rounded-xl border border-gray-800 w-full max-w-md p-6" onClick={e => e.stopPropagation()}><h3 className="text-lg font-semibold mb-4">Create Account</h3><div className="space-y-3"><div><label className="block text-sm text-gray-400 mb-1">Name</label><input type="text" value={newAccName} onChange={e => setNewAccName(e.target.value)} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:border-cyan-500 focus:outline-none" /></div><div><label className="block text-sm text-gray-400 mb-1">Email</label><input type="email" value={newAccEmail} onChange={e => setNewAccEmail(e.target.value)} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:border-cyan-500 focus:outline-none" /></div><div><label className="block text-sm text-gray-400 mb-1">Password</label><input type="password" value={newAccPassword} onChange={e => setNewAccPassword(e.target.value)} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:border-cyan-500 focus:outline-none" /></div><div><label className="block text-sm text-gray-400 mb-1">Role</label><select value={newAccRole} onChange={e => setNewAccRole(e.target.value)} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm"><option value="user">User</option><option value="admin">Admin</option></select></div><button onClick={handleCreateAccount} className="w-full py-2 bg-cyan-600 hover:bg-cyan-500 rounded-lg font-medium text-sm">Create Account</button></div></div></div>)}</div>)} {/* Issue Detail Modal */}
    {selectedIssue && (
      <div className="fixed inset-0 z-[80] flex items-center justify-center p-4" onClick={() => setSelectedIssue(null)}><div className="absolute inset-0 bg-black/60" />
        <div className="relative bg-gray-900 rounded-xl border border-gray-800 w-full max-w-2xl max-h-[90vh] overflow-y-auto p-4 sm:p-6" onClick={e => e.stopPropagation()}>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm mb-4">
            ...
            <div><span className="text-gray-500">Points: </span><span>{selectedIssue.points}</span></div><div><span className="text-gray-500">Due Date: </span><span>{selectedIssue.dueDate || 'Not set'}</span></div><div><span className="text-gray-500">Due Time:</span> <span className="text-white">{selectedIssue.dueTime ? ` ${selectedIssue.dueTime}` : 'Not set'}</span></div>
          </div>
