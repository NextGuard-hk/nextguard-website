      ...
      <div className="qd-hdr">
        ...
        <div className="qd-hdr-r">
          ...
          {!editing && <>
            ...
            <button onClick={()=>router.push(`/qt/${id}/pdf`)} className="qd-b" style={{background:'#374151'}}>PDF</button> <button onClick={() => router.push(`/qt/${id}/invoice`)} className="qd-b" style={{background:'#059669'}}>Invoice</button> <button onClick={()=>{if(confirm('Delete this quotation?')){fetch('/api/qt-quotations/'+id,{method:'DELETE'}).then(r=>r.json()).then(d=>{if(d.success)router.push('/qt');else alert(d.error||'Failed')})}}} className="qd-b" style={{background:'#dc2626'}}>Delete</button>
          </>}
        </div>
      </div>
