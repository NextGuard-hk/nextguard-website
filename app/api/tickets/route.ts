...
async function saveData(data: any) {
  await fetch(NPOINT_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) })
}
  ...
  try {
    ...
    if (isAdmin) {
      ...
      const avgResTime = resolved.length > 0 ? resolved.reduce((s: number, t: any) => {
        return s + (new Date(t.resolvedAt || t.updatedAt).getTime() - new Date(t.createdAt).getTime())
      }, 0) / resolved.length / 3600000 : 0
      kpi = {
        totalTickets: all.length, openTickets: open.length, resolvedTickets: resolved.length,
        breachedResponse: breachedResponse.length, breachedResolution: breachedResolution.length,
        avgResolutionHours: Math.round(avgResTime * 10) / 10,
        byPriority: { critical: open.filter((t: any) => t.priority === "critical").length, high: open.filter((t: any) => t.priority === "high").length, medium: open.filter((t: any) => t.priority === "medium").length, low: open.filter((t: any) => t.priority === "low").length },
        byStatus: { new: all.filter((t: any) => t.status === "new").length, open: all.filter((t: any) => t.status === "open").length, in_progress: all.filter((t: any) => t.status === "in_progress").length, waiting_customer: all.filter((t: any) => t.status === "waiting_customer").length, waiting_vendor: all.filter((t: any) => t.status === "waiting_vendor").length, resolved: all.filter((t: any) => t.status === "resolved").length, closed: all.filter((t: any) => t.status === "closed").length },
      }
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { customerName, customerEmail, customerCompany, subject, description, category, priority, product, version, attachments, environment, stepsToReproduce } = body
    ...
    // Process attachments - store metadata, keep base64 data
    const processedAttachments = (attachments || []).map((a: any) => ({
      name: a.name, size: a.size, type: a.type, data: a.data,
    }))
    const ticket = {
      ...
      timeline: [{ type: "created", message: `Ticket created${processedAttachments.length > 0 ? ` with ${processedAttachments.length} attachment(s)` : ""}`, by: customerName, at: now }],
      comments: [],
    }
    const data = await getData()
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (action === "update_status" && isAdmin) {
      ...
      ticket.timeline.push({ type: "status", message: `Status: ${oldStatus} -> ${status}`, by: body.staffName || "Support", at: now })
    ...
    if (action === "update_priority" && isAdmin) {
      ...
      ticket.timeline.push({ type: "priority", message: `Priority: ${oldPriority} -> ${priority}`, by: body.staffName || "Support", at: now })
    }
