...
function getFeedStatuses(): FeedStatus[] {
  return [
    ...
    },
    {
      ...
      apiKeyConfigured: !!process.env.GOOGLE_SAFE_BROWSING_API_KEY,
      tier: 'free',
      dailyLimit: 10000,
      usedToday: 0,
      lastChecked: new Date().toISOString(),
    },
    {
      ...
      apiKeyConfigured: !!process.env.CLOUDFLARE_API_TOKEN,
      tier: process.env.CLOUDFLARE_TIER || 'free',
      dailyLimit: process.env.CLOUDFLARE_TIER === 'enterprise' ? 100000 : 1000,
      usedToday: 0,
      lastChecked: new Date().toISOString(),
    },
  ];
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (provider === 'all' || provider === 'google_safe_browsing') {
      try {
        const { queryGoogleSafeBrowsing } = await import('@/lib/commercial-feeds');
        const r = await queryGoogleSafeBrowsing(testDomain);
        results.google_safe_browsing = { status: 'ok', data: r ? 'connected' : 'no_key' };
      } catch (e: any) { results.google_safe_browsing = { status: 'error', message: e.message }; }
    }
    if (provider === 'all' || provider === 'cloudflare_radar') {
      try {
        const { queryCloudflareRadar } = await import('@/lib/commercial-feeds');
        const r = await queryCloudflareRadar(testDomain);
        results.cloudflare_radar = { status: 'ok', data: r ? 'connected' : 'no_key' };
      } catch (e: any) { results.cloudflare_radar = { status: 'error', message: e.message }; }
    }
