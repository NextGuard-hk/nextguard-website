...
// L1: In-memory cache with larger capacity
...
const DEFAULT_POLICY: Record<string, string> = {
  ...
  'exploit/attack tools': 'Block', 'scam/fraud': 'Block',
  'proxy/anonymizer': 'Warn', 'vpn services': 'Warn', 'cryptocurrency': 'Warn',
  'social networking': 'Warn', 'streaming video': 'Warn', 'games': 'Warn',
...
async function evaluateUrl(inputUrl: string, userId: string | null = null) {
  ...
  if (confidence < 70 && !threatIntel.isMalicious) {
    ...
    if (cfIntelResult) {
      ...
      for (const c of cfIntelResult.categories) { if (!allCategories.includes(c)) { allCategories.push(c); if (confidence < 75) { confidence = 75; categorySource = 'cloudflare-intel'; } } }
    }
  }
