...
const ACTION_PRIORITY: Record<string, number> = { 'Block': 4, 'Warn': 3, 'Isolate': 2, 'Allow': 1 };
...
async function evaluateUrl(inputUrl: string, userId: string | null = null) {
 ...
 if (isAIClassificationAvailable() && (allCategories.length === 0 || (allCategories.length === 1 && allCategories[0] === 'Uncategorized'))) { try { const aiResult = await classifyUrlWithAI(domain); if (aiResult.primaryCategory && aiResult.primaryCategory !== 'Uncategorized' && aiResult.confidence >= 50) { allCategories.push(aiResult.primaryCategory); categorySource = 'ai-' + aiResult.source; confidence = aiResult.confidence; aiClassified = true; } } catch {} }
