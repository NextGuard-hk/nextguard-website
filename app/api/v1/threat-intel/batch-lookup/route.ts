...
async function lookupSingle(indicator: string, mode: string) {
  ...
  if (mode === 'db' || mode === 'hybrid') {
    try {
      ...
      if (dbResult.hits.length > 0) {
        ...
        const urlCats = await lookupUrlCat(trimmed);
        const allCats = [...new Set([...dbResult.hits.flatMap(h => h.categories), ...urlCats])];
        const cats = allCats.length > 0 ? allCats : categorizeUrl(trimmed);
        return {
          ...
          engine: 'turso-db-v5.3', lookup_ms: Date.now() - start,
        };
      }
    } catch {}
  }
  ...
  try {
    ...
    const cats = result.categories.length > 0 ? result.categories : categorizeUrl(trimmed);
    return {
      ...
      engine: 'osint-live-v4.8', lookup_ms: Date.now() - start,
    };
  } catch {
    return {
