...
function checkRateLimits(token: DownloadToken): { allowed: boolean; error?: string } {
  ...
  const dl = token.downloads || []
  const hourlyCount = dl.filter(d => new Date(d.timestamp) > oneHourAgo).length
  const dailyCount = dl.filter(d => new Date(d.timestamp) > oneDayAgo).length
  const dailyBytes = dl.filter(d => new Date(d.timestamp) > oneDayAgo).reduce((s, d) => s + (d.size || 0), 0)
  const maxHour = token.maxDownloadsPerHour || 8
  const maxDay = token.maxDownloadsPerDay || 15
  const maxBytes = token.maxBytesPerDay || 15 * 1024 * 1024 * 1024
  if (hourlyCount >= maxHour) return { allowed: false, error: `Hourly download limit reached (${maxHour}/hour). Please try again later.` }
  if (dailyCount >= maxDay) return { allowed: false, error: `Daily download limit reached (${maxDay}/day). Please try again tomorrow.` }
  if (dailyBytes >= maxBytes) return { allowed: false, error: `Daily bandwidth limit reached (${(maxBytes/(1024*1024*1024)).toFixed(0)}GB/day). Please try again tomorrow.` }
  return { allowed: true }
}
...
async function recordTokenDownload(tokenStr: string, key: string, size: number, ip: string) {
  ...
  if (found) {
    ...
    found.downloads = found.downloads.filter(d => new Date(d.timestamp) > ago30)
...
async function scanWithVirusTotal(key: string): Promise<{ safe: boolean; message: string }> {
  ...
  async function checkStorageAndNotify() {
    try {
      ...
      do {
        ...
        for (const obj of list.Contents || []) { totalSize += obj.Size || 0 }
        continuationToken = list.NextContinuationToken
      } while (continuationToken)
      if (totalSize >= STORAGE_WARN_BYTES) {
        const totalGB = (totalSize / (1024 * 1024 * 1024)).toFixed(2)
        const resendApiKey = process.env.RESEND_API_KEY
        if (!resendApiKey) { console.error('Storage warning: ' + totalGB + 'GB used but no RESEND_API_KEY'); return }
        await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            ...
            html: `<p>R2 bucket <b>${BUCKET}</b> has reached <b>${totalGB} GB</b>. Please review.</p>`,
          }),
        })
        await writeLog({ type: 'system', action: 'storage-warning', size: totalSize.toString(), message: totalGB + 'GB used' })
      }
  ...
  try {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const resultRes = await fetch(`https://www.virustotal.com/api/v3/analyses/${analysisId}`, { headers: { 'x-apikey': VT_API_KEY } })
      ...
      if (status === 'completed') {
        ...
        if (malicious > 0 || suspicious > 0) return { safe: false, message: `Threat detected: ${malicious} malicious, ${suspicious} suspicious` }
        return { safe: true, message: 'Scan clean' }
      }
    }
    return { safe: true, message: 'Scan timeout, allowing upload' }
  } catch (e: any) { return { safe: true, message: `Scan error: ${e.message}` } }
}
...
export async function GET(req: NextRequest) {
  ...
  if (action === 'setup-cors') {
    ...
    if (!accountId || !cfApiToken) return NextResponse.json({ error: 'Missing R2_ACCOUNT_ID or CLOUDFLARE_API_TOKEN env var.' }, { status: 500 })
    try {
      const corsUrl = `https://api.cloudflare.com/client/v4/accounts/${accountId}/r2/buckets/${BUCKET}/cors`
      const corsPolicy = { rules: [{ allowed: { origins: ['https://next-guard.com'], methods: ['GET', 'PUT', 'POST', 'HEAD'], headers: ['*'] }, exposeHeaders: ['ETag'], maxAgeSeconds: 86400 }] }
      const res = await fetch(corsUrl, { method: 'PUT', headers: { 'Authorization': `Bearer ${cfApiToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify(corsPolicy) })
      const data = await res.json()
      if (data.success) return NextResponse.json({ success: true, message: 'CORS configured for R2 bucket' })
      else return NextResponse.json({ error: data.errors?.[0]?.message || 'Cloudflare API error', details: data.errors }, { status: 500 })
    } catch (e: any) { return NextResponse.json({ error: 'Internal server error' }, { status: 500 }) }
  }
  ...
  if (action === 'multipart-presign') {
    ...
    try {
      const url = await getSignedUrl(S3, new UploadPartCommand({ Bucket: BUCKET, Key: key, UploadId: uploadId, PartNumber: parseInt(partNumber) }), { expiresIn: 3600 })
  ...
  // List all files recursively - admin only
  ...
  // List files (public for download users/token holders, all for admin)
  if (!action || action === 'list') {
    const tokenParam = searchParams.get('token') || req.headers.get('x-download-token') || ''
    ...
    try {
      ...
      if (!admin) { prefix = PUBLIC_PREFIX + prefix.replace(/^public\//, '') }
      const data = await S3.send(new ListObjectsV2Command({ Bucket: BUCKET, Prefix: prefix, Delimiter: '/' }))
      const folders = (data.CommonPrefixes || []).map(p => ({ name: p.Prefix?.replace(prefix, '').replace(/\/$/, '') || '', path: p.Prefix || '', type: 'folder' as const }))
      const files = (data.Contents || []).filter(f => f.Key !== prefix).map(f => ({ name: f.Key?.replace(prefix, '') || '', path: f.Key || '', size: f.Size || 0, lastModified: f.LastModified?.toISOString() || '', type: 'file' as const }))
      return NextResponse.json({ items: [...folders, ...files] })
    } catch (e: any) { return NextResponse.json({ error: 'Internal server error' }, { status: 500 }) }
  }

  // Download file with token auth + rate limiting
  if (action === 'download') {
    const dlTokenParam = searchParams.get('token') || req.headers.get('x-download-token') || ''
    ...
    try {
      ...
      if (budgetRes.ok) {
        ...
        if (budget.budgetExceeded) {
          ...
          return NextResponse.json({ error: 'Download temporarily unavailable - monthly bandwidth budget exceeded.', budgetExceeded: true }, { status: 503 })
        }
      }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!file || key === 'unknown') return NextResponse.json({ error: 'Missing file or key' }, { status: 400 })
    const buffer = Buffer.from(await file.arrayBuffer())
    await S3.send(new PutObjectCommand({ Bucket: BUCKET, Key: key, Body: buffer, ContentType: file.type || 'application/octet-stream' }))
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (tokenStr) {
      ...
      if (!tokenValidation.valid) return NextResponse.json({ error: tokenValidation.error || 'Invalid token' }, { status: 401 })
      if (acceptDisclaimer && tokenValidation.token) {
        const allTokens = await getTokens()
        const found = allTokens.find(t => t.token === tokenStr)
        if (found) { found.disclaimerAccepted = true; await saveTokens(allTokens) }
      }
      const res = NextResponse.json({ success: true, tokenAuth: true, company: tokenValidation.token?.company })
      res.cookies.set('download_session_token', DOWNLOAD_PASSWORD, { httpOnly: true, secure: true, sameSite: 'strict', path: '/', maxAge: 60 * 60 * 24 })
      return res
    }

    // Password-based authentication (fallback)
    const pw = body.password || ''
    if (pw !== DOWNLOAD_PASSWORD) return NextResponse.json({ error: 'Invalid password' }, { status: 401 })
    const res = NextResponse.json({ success: true })
    res.cookies.set('download_auth', DOWNLOAD_PASSWORD, { httpOnly: true, secure: true, sameSite: 'strict', path: '/', maxAge: 60 * 60 * 24 })
    return res
