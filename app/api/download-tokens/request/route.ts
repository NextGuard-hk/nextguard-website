...
export async function POST(req: NextRequest) {
  try {
    ...
    // Admin: approve or reject via POST body     if (body.action === 'approve' && isAdmin(req)) {       const id = body.id       const requests = await getRequests()       const found = requests.find((r: any) => r.id === id)       if (!found) return NextResponse.json({ error: 'Request not found' }, { status: 404 })       if (found.status !== 'pending') return NextResponse.json({ error: 'Already processed' }, { status: 400 })       const tokenType = found.type || 'customer'       const defaultExpiry = tokenType === 'poc' ? 90 : 180       const expiresAt = new Date(Date.now() + defaultExpiry * 24 * 60 * 60 * 1000).toISOString()       const newToken = { id: 'dt-' + Date.now(), token: generateToken(), company: found.company, contact: found.contact, email: found.email, type: tokenType, scope: ['software','hotfix','manual','iso'], createdAt: new Date().toISOString(), expiresAt, active: true, maxDownloadsPerHour: 8, maxDownloadsPerDay: 15, maxBytesPerDay: 15*1024*1024*1024, downloads: [], disclaimerAccepted: false }       const tokens = await getTokens()       tokens.push(newToken)       await saveTokens(tokens)       found.status = 'approved'; found.reviewedAt = new Date().toISOString(); found.tokenId = newToken.id       await saveRequests(requests)       await sendEmail(found.email,'Your NextGuard Download Access Has Been Approved',`<p>Dear ${found.contact}, your token is: <strong>${newToken.token}</strong></p>`)       return NextResponse.json({ success: true, token: newToken.token })     }     if (body.action === 'reject' && isAdmin(req)) {       const id = body.id       const rejectReason = body.reason || 'Your request does not meet our current access criteria.'       const requests = await getRequests()       const found = requests.find((r: any) => r.id === id)       if (!found) return NextResponse.json({ error: 'Request not found' }, { status: 404 })       if (found.status !== 'pending') return NextResponse.json({ error: 'Already processed' }, { status: 400 })       found.status = 'rejected'; found.reviewedAt = new Date().toISOString(); found.rejectReason = rejectReason       await saveRequests(requests)       return NextResponse.json({ success: true })     }     const { company, contact, email, type, reason } = body
