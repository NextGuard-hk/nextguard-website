...
def get_macos_name(version: str) -> str:
    """Map macOS version to release name."""
...
def get_device_id() -> str:
    """Generate persistent device ID from hardware serial."""
    try:
        result = subprocess.run(
...
# ============================================================
class APIClient:
    """HTTP client for NextGuard Management Console API."""
    ...
    def register(self, system_info: dict) -> bool:
        """Register agent with management console."""
    ...
    def sync_policies(self) -> Optional[List[dict]]:
        """Sync policies via agent-sync endpoint."""
        resp = self._request('POST', '/agent-sync', {
            'tenantId': self.tenant_id, 'agentId': self.agent_id,
            'action': 'heartbeat'
        })
        if resp and resp.get('success'):
    ...
    def report_incident(self, incident: dict) -> bool:
        """Report DLP incident to console."""
        payload = {
            'agentId': self.agent_id, 'tenantId': self.tenant_id,
...
# ============================================================
class PolicyEngine:
    """Evaluate DLP policies against file operations."""
    ...
    def save_cache(self):
        """Save policies to disk for offline use."""
        try:
    ...
    def evaluate(self, event: dict) -> Optional[dict]:
        """Evaluate event against all policies. Returns matched policy or None."""
        ...
        for policy in self.policies:
            ...
            if not conditions:
                # No conditions = match all on this channel
                return policy
...
class FileSystemMonitor:
    """Monitor file system events using macOS log stream."""

    SENSITIVE_DIRS = [
        Path.home() / 'Documents',
        Path.home() / 'Desktop',
        Path.home() / 'Downloads',
        Path('/Volumes'),  # USB drives
    ]
    ...
    def __init__(self, policy_engine: PolicyEngine, api: APIClient):
        ...
        self.api = api
        self.running = False
        self._watched: Dict[str, float] = {}  # debounce
    ...
    def _monitor_loop(self):
        """Poll for file changes in sensitive directories."""
        while self.running:
            try:
...
# ============================================================
class USBMonitor:
    """Monitor USB device connections on macOS."""
    ...
    def _get_volumes(self) -> List[str]:
        ...
        if not volumes_path.exists():
            return []
        return [v.name for v in volumes_path.iterdir() if v.is_dir()]
            ...
            try:
                ...
                if content and content != self._last_content:
                    ...
                    matched = self.engine.evaluate({
                        'channel': 'clipboard', 'content': content, 'fileName': ''
                    })
                    if matched:
                        ...
                        self.api.report_incident({
                            ...
                            'channel': 'clipboard', 'riskScore': 65,
...
class NextGuardAgent:
    ...
    def start(self):
        ...
        # Main loop: heartbeat + policy sync
        last_heartbeat = 0
        last_sync = 0
        try:
            while self.running:
...
def main():
    ...
    parser.add_argument('--server', default='https://www.next-guard.com',
