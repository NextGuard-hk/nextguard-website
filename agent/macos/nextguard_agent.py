...
class APIClient:
  ...
  def register(self, system_info):
    ...
    if resp and resp.get('success'):
      ...
      log.info(f'Registered as {self.agent_id} (token: {"yes" if self.token else "no"}, policies: {len(policies)})')
      return resp
...
class PolicyEngine:
  ...
  def _check(self, cond, content, file_name):
    ...
    if not cond.get('isCaseSensitive', False): text = text.lower(); value = value.lower()
    op = cond.get('operator', 'contains')
    if op == 'contains': return value in text
    if op == 'matches':
      import re
      try: return bool(re.search(value, text))
      except: return False
...
class NextGuardAgent:
  ...
  def start(self):
    ...
    if not reg_resp:
      ...
      policies = reg_resp.get('policies', [])
      if policies:
        log.info(f'Got {len(policies)} policies from registration')
